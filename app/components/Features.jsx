...
function Card({ feat }) {
  ...
  return (
    <motion.article ref={ref}
      ...
      transition={{ duration: 0.6, delay: feat.delay, ease: [0.16, 1, 0.3, 1] }}
      whileHover={{ y: -6, borderColor: `${feat.color}55`, boxShadow: `0 16px 48px ${feat.color}10` }}
      style={{
      ...
      <motion.div whileHover={{ scale: 1.1, rotate: 6 }}
        style={{
          ...
          background: `${feat.color}10`, color: feat.color,
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20,
          border: `1px solid ${feat.color}18`,
        }}>
        {feat.icon}
      </motion.div>
      <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 8, color: '#f1f5f9' }}>{feat.title}</h3>
      <p style={{ fontSize: 13, color: '#555', lineHeight: 1.7 }}>{feat.text}</p>
      <div style={{ marginTop: 16, fontSize: 12, color: feat.color, display: 'flex', alignItems: 'center', gap: 4 }}>Ko'proq <span>→</span></div>
    </motion.article>
  )
}
...
export default function Features() {
  const [ref, inView] = useInView({ triggerOnce: true, threshold: 0.1 })
  return (
    <section id="features" aria-label="Imkoniyatlar" style={{ padding: '96px 5%', position: 'relative', zIndex: 2 }}>
      {/* Background glow */}
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative' }}>
        <motion.header ref={ref} style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <motion.h2 initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1 }}
            style={{ fontSize: 'clamp(26px,4vw,48px)', fontWeight: 900, marginBottom: 14, letterSpacing: '-1.5px', lineHeight: 1.1 }}>
          ...
          <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.2 }}
            style={{ fontSize: 16, color: '#555', maxWidth: 480, margin: '0 auto', lineHeight: 1.7 }}>
